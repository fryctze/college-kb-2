...
public class gantiKataTarzan extends AppCompatActivity {

    private static final String TAG = "hehe";

    EditText et0, et1, et2, et3, et4, et5, et6, et7, et8, et9, et10;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_ganti_kata_tarzan);
    ...
    private void readFromFile(){
        ...
        String a9 = et9.getText().toString();
        String a10 = et10.getText().toString();
            ...
            for (int i = 0; i < line.length(); i++) {
                if (line.charAt(i) == '['){
                    switch (line.charAt(i+1)){
                        case '0':
                            if (line.charAt(i+2) == '1'){ // 10
                                Log.d(TAG, "readFromFile: "+a10);
                                for (int j = 0; j < a10.length(); j++) {
                                    newline.append(a10.charAt(j));
                                }
                            } else { // 0
                        ...
                        case '7':
                            ...
                            break;
                        case '8':
                            Log.d(TAG, "readFromFile: "+a8);
                            for (int j = 0; j < a8.length(); j++) {
                                newline.append(a8.charAt(j));
                            }
                            foundBracket=true;
                            break;
                        case '9':
                            Log.d(TAG, "readFromFile: "+a9);
                            for (int j = 0; j < a9.length(); j++) {
                                newline.append(a9.charAt(j));
                            }
                            foundBracket=true;
                            break;
                    }
